...
@section('content')
    ...
    <div class="grid grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg border border-slate-200 p-6 col-span-1">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-4">Información</h2>
            <dl class="space-y-4">
                <div>
                    <dt class="text-xs text-slate-500">Nombre completo</dt>
                    <dd class="mt-0.5 text-sm font-medium text-slate-800">{{ $employee->first_name }} {{ $employee->last_name }}</dd>
                </div>
                <div>
                ...
                <div>
                    ...
                    <dd class="mt-0.5 text-sm text-slate-800">{{ $employee->created_at->format('d/m/Y') }}</dd>
                </div>
            </dl>
        </div>
            ...
            <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Serial</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Modelo</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Departamento</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($employee->computers as $computer)
                            <tr class="hover:bg-slate-50">
                                <td class="px-5 py-3 text-sm font-mono text-slate-700">{{ $computer->serial }}</td>
                                <td class="px-5 py-3 text-sm text-slate-700">
                                    {{ $computer->brandModel->brand->name }} {{ $computer->brandModel->name }}
                                </td>
                                <td class="px-5 py-3 text-sm text-slate-500">{{ $computer->department?->name ?? '—' }}</td>
                                <td class="px-5 py-3 text-right">
                                    <a href="{{ route('computers.show', $computer) }}" class="text-xs text-indigo-600 hover:text-indigo-800">Ver equipo →</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
